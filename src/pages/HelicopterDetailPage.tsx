...
export const HelicopterDetailPage: React.FC = () => {
  ...
  // Track helicopter view with comprehensive Facebook Pixel tracking
  useEffect(() => {
    if (helicopter) {
      // Track page view
      FacebookPixelTracker.trackPageView(`Helicopter - ${helicopter.name}`);

      // Track helicopter view with specific data
      FacebookPixelTracker.trackHelicopterView({
        helicopter_id: helicopter.id,
        helicopter_name: helicopter.name,
        passengers_count: helicopter.capacity,
        total_price: helicopter.pricePerHour,
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      // Track successful helicopter search/load
      if (found) {
        FacebookPixelTracker.trackSearch(found.name, 1);
      }
    }, 500);
  }, [id]);
  ...
  // Track booking initiation with comprehensive Facebook Pixel tracking
  const handleBookNow = (e?: React.MouseEvent) => {
    ...
    // Track booking initiation with comprehensive data
    FacebookPixelTracker.trackBookingStarted({
      helicopter_id: helicopter.id,
      helicopter_name: helicopter.name,
      flight_date: selectedDate,
      passengers_count: 1, // Default passengers
      total_price: helicopter.pricePerHour,
      customer_email: '',
      customer_phone: '',
      booking_reference: ''
    ...
    // Track as a lead (potential conversion)
  ...
  const handleWishlistToggle = () => {
    ...
    if (helicopter) {
      if (newWishlistState) {
        // Track adding to wishlist as a lead
        FacebookPixelTracker.trackLead({
          content_name: helicopter.name,
          content_type: 'helicopter',
          content_ids: [helicopter.id],
  ...
  const handleShare = () => {
    if (helicopter) {
      FacebookPixelTracker.trackSocialClick('share');
      
      // Native share if available, otherwise copy to clipboard
      if (navigator.share) {
        navigator.share({
          title: helicopter.name,
          text: `Check out this amazing helicopter experience: ${helicopter.name}`,
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
              <div className="relative">
                ...
                <div className="absolute top-4 right-4 flex space-x-2">
                  ...
                  <button 
                    onClick={handleShare}
                    className="p-2 rounded-full bg-white/80 text-navy-700 hover:bg-white backdrop-blur-sm transition-colors duration-200"
                  >
              ...
              <div className="border-t mt-6 pt-6">
                ...
                <div className="space-y-2">
                  <a href="tel:+17028158697" className="flex items-center text-navy-600 hover:text-gold-600">
                    ...
                    <span className="text-sm">+17028158697</span>
                  </a>
